  ...
  if (isSuccess) {
    ...
    return (
      <div className="bg-[var(--surface)] rounded-lg border border-[var(--border-subtle)] p-8">
        ...
        {/* Refresh Warning */}
        <div className="flex items-start gap-2 p-3 bg-[var(--warning-subtle)] rounded-lg border border-[var(--warning)] mt-4">
          <div className="w-4 h-4 rounded-full bg-[var(--warning)] flex items-center justify-center shrink-0 mt-0.5">
            <svg className="w-2.5 h-2.5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
            </svg>
          </div>
          <p className="text-xs text-[var(--foreground)]">
            Website will refresh information soon
          </p>
        </div>
      </div>
    );
